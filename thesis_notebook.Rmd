...
filter_callers <- function(dataset) {
  ...
  temp <- 
    filter(dataset, 
           !B_NUMBER %in% c(incoming_calls$B_NUMBER, outgoing_call$A_NUMBER), 
           !A_NUMBER %in% c(incoming_calls$B_NUMBER, outgoing_call$A_NUMBER)
    )
  return(temp)
}
...
# show plot
elbow_plot(kmean_ranged, "Clusterset ranged")
# safe plot
png("elbow_kmeans_final2.png", width = 500,  height = 800, res = 100)
par(mfrow=c(1,1))
elbow_plot(kmean_ranged, "Clusterset ranged")
dev.off()
...
clara_fun <- function(dataset, max_cluster) {
  ...
  result.clara <- foreach(i=2:max_cluster) %dopar% {
...
# run clara function
clara_ranged <- clara_fun(clusterset_ranged[,-1], 15)
```
### Evalute k-medoid
...
plot(2:15, sapply(clara_ranged, function(x) x$silinfo$avg.width), type="b", xlab="Number of Clusters", ylab="Average silhouette width", main = "CLARA (k-mediod)")
...
skmeans_fun <- function(dataset, max_cluster) {
  ...
  result.skmeans <- foreach(i= 2:max_cluster) %dopar% {
...
skmeans_ranged <- skmeans_fun(clusterset_ranged[,-1], 15)
...
distplot_clusterset_scaled <- kNNdistplot(clusterset_ranged[,-1], k =  10)
...
ch_plot <- function(dataset, datacluster, plotname, method) {
  ...
  png(paste0(deparse(substitute(datacluster)),"_final.png"), width = 800,  height = 500, res = 100)
  par(mfrow=c(1,2))
...
# Kmeans
# plot function
ch_plot(clusterset_ranged, kmean_ranged, "Clusterset ranged kmean", method = "kmean")
ch_plot(clusterset_ranged, clara_ranged, "Clusterset ranged clara", method = "clara")


# Get stats for a specific cluster
sink("db_ch_statistics.txt")
measure_cluster(clusterset_ranged, kmean_ranged[[8]]$cluster)
measure_cluster(clusterset_ranged, kmean_ranged[[9]]$cluster)
measure_cluster(clusterset_ranged, kmeansrun_ranged$cluster)
measure_cluster(clusterset_ranged, pamk_ranged$pamobject$clustering)
measure_cluster(clusterset_ranged, clara_ranged[[7]]$clustering)
measure_cluster(clusterset_ranged, clara_ranged[[8]]$clustering)
sink()
...
# plot 
cluster_analyse(clusterset, kmean_ranged[[10]]$cluster)
cluster_analyse(clusterset, kmean_ranged[[8]]$cluster)
cluster_analyse(clusterset, clara_ranged[[7]]$clustering)
