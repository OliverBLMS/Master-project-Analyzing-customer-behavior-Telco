...
stats_boxplot
# Look at density of the long tail.
# cdr1$CALL_ACTUAL_DURATION[cdr1$CALL_ACTUAL_DURATION >= stats_boxplot[5]] %>% hist()

# addes maximum as endpoint for the breaks
cdr1$breaks <- cut(cdr1$CALL_ACTUAL_DURATION, breaks=c(stats_boxplot, max(cdr1$CALL_ACTUAL_DURATION)) ,labels=F, include.lowest = T, right = T)
...
clusterset <- 
  dplyr::select(finalset,
                ...
                nr_call_HOME_NET_perc,
                nr_call_OTHER_NET_perc,
                total_call_time_NON_PEAK_perc,
                total_call_time_PEAK_perc,
                total_call_time_WEEK_perc,
                total_call_time_WEEKEND_perc,
                total_call_time_HOME_NET_perc,
                total_call_time_OTHER_NET_perc,
                ave_call_time_NON_PEAK,
                ave_call_time_PEAK,
                ave_call_time_WEEK,
                ave_call_time_WEEKEND,
                ave_call_time_HOME_NET,
                ave_call_time_OTHER_NET,
                degree_all,
                degree_in,
                degree_out,
                betweenness,
...
# dont go with percentages, go with total call time instead
clusterset2 <- 
  dplyr::select(finalset,
                nr_unique_calls,
                recency_min,
                ...
                nr_call_HOME_NET,
                nr_call_OTHER_NET,
                total_call_time_NON_PEAK,
                total_call_time_PEAK,
                total_call_time_WEEK,
                total_call_time_WEEKEND,
                total_call_time_HOME_NET,
                total_call_time_OTHER_NET,
                ave_call_time_NON_PEAK,
                ave_call_time_PEAK,
                ave_call_time_WEEK,
                ave_call_time_WEEKEND,
...
### Scale everything between value of [0-1]

```{r}
# unitization with zero minimum: make  function
range01 <- function(x){(x-min(x))/(max(x)-min(x))}

# check with features need to be scaled, max value is not 1
scale_features <- names(clusterset)[apply(clusterset, 2, function(x) max(x) != 1)]
# make set
# summary(clusterset)
clusterset_scaled <- 
  clusterset %>%
  mutate_each_(funs((.-min(.))/(max(.)-min(.))), scale_features)
# summary(clusterset_scaled)

# summary(clusterset2)
# check with features need to be scaled, max value is not 1
scale_features <- names(clusterset2)[apply(clusterset2, 2, function(x) max(x) != 1)]
clusterset2_scaled <- 
  clusterset2 %>%
  mutate_each_(funs((.-min(.))/(max(.)-min(.))), scale_features)
# summary(clusterset2_scaled)
...
clusterset_no_cor <- delete_correlation(clusterset_scaled, 0.80, FALSE)
clusterset2_no_cor <- delete_correlation(clusterset2_scaled, 0.80, FALSE)
```
...
not feasable to calculate distance matrics on this size
```{r}
library(factoextra)
library(clustertend)
library(seriation)
...
# not feasable
# hopkins(clusterset_scaled, n = nrow(clusterset_scaled)-1)

# not feasable
# df <- dist(clusterset_scaled)
```
...
kmean_fun <- function(dataset, max_cluster) {
  ...
  result.kmean <- foreach(i=1:max_cluster) %dopar% {
    km <- kmeans(dataset, centers = i, iter.max = 15);
    return(km)
  }
  stopImplicitCluster()
  stopCluster(cl)
...
# normal set, perc vs abso
kmean_orig_noscale <- kmean_fun(clusterset, 10)
kmean_set2_noscale <- kmean_fun(clusterset2, 10)
# scaled set, perc vs abso
kmean_scaled <- kmean_fun(clusterset_scaled, 10)
kmean_scaled_set2 <- kmean_fun(clusterset2_scaled, 10)
# usage set, perc vs abso
kmean_usage <- kmean_fun(clusterset_usage, 10)
kmean_usage_set2 <- kmean_fun(clusterset2_usage, 10)
# usage scaled set, perc vs abso
kmean_usage_scaled <- kmean_fun(clusterset_usage_scaled, 10)
kmean_usage_scaled_set2 <- kmean_fun(clusterset2_usage_scaled, 10)
# normal set, NO COR, perc vs abso
kmean_no_cor <- kmean_fun(clusterset_no_cor, 10)
kmean_no_cor_set2 <- kmean_fun(clusterset2_no_cor, 10)

elbow_plot <- function(dataset, plotname) {
  plot(1:10, sapply(dataset, function(x) x$tot.withinss), type="b", xlab="Number of Clusters", ylab="Within groups sum of squares", main = plotname)
}
par(mfrow=c(2,5))

elbow_plot(kmean_orig_noscale, "Clusterset perc")
elbow_plot(kmean_set2_noscale, "Clusterset no perc")
elbow_plot(kmean_scaled, "Clusterset scaled, perc")
elbow_plot(kmean_scaled_set2, "Clusterset scaled, no perc")
elbow_plot(kmean_usage, "Clusterset usage features only, perc")
elbow_plot(kmean_usage_set2, "Clusterset usage features only, no perc")
elbow_plot(kmean_usage_scaled, "Clusterset usage features , scaled, perc")
elbow_plot(kmean_usage_scaled_set2, "Clusterset usage features, scaled, no perc")
elbow_plot(kmean_no_cor, "Clusterset no cor, perc")
elbow_plot(kmean_no_cor_set2, "Clusterset  no cor, no perc")
...
pamk_fun <- function(dataset, iterations) {
  ...
  result.pamk <- foreach(i=1:iterations, .packages = 'fpc') %dopar% {
    km <- pamk(dataset, criterion="multiasw");
    return(km)
  }
  stopImplicitCluster()
  stopCluster(cl)
...
pamk_scaled <- pamk(clusterset_scaled, usepam = FALSE)
```


## Measure Cluster

difference internal / external cluster criteria / validation
