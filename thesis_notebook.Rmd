...
# Feature engineering: Creating breakdown structure
...
> do this just before clustering. When the finalset dataset is created.
...
# Feature engineering: usage features
...
# Feature engineering: Social network Analysis
...
# Data transformation: Blend it all together
...
* include extra data cleaning step

```{r}
load(file = "finalset.RData")


## DATA CLEANING SUBSTEP ##
#------------------------#

# Only hold full 2 week groups
# Visual
tidy(table(finalset$mygroup)) %>% 
  ggplot(., aes(x = Var1, y = Freq/1000)) + geom_bar(stat = "identity") + ylab("number of observations (k)")+ xlab("Groups: two-week interval") +
  theme_bw() + theme_minimal()

finalset <-
  filter(finalset, mygroup %in% c("18","20","22","24","26","28"))


# Keep those customers who are presents in several periods (make a threshold)
count_customers <-
  finalset %>%
  dplyr::select(mygroup, A_NUMBER) %>%
  group_by(A_NUMBER) %>%
  summarise(nr = n_distinct(mygroup)) %>%
  filter(nr > 3)
...
finalset_key <- 
  finalset %>%
  filter(A_NUMBER %in% count_customers$A_NUMBER) %>%
...
# Take log(x+1) from features (# Add +1 so that 0 values are not replaced by -inf)
clusterset_log <-
  clusterset %>%
  mutate_at(
...
high_correlated = findCorrelation(cor_set, cutoff= 0.8, exact = TRUE, names = TRUE) 
high_correlated
```
...
# Make clusterset only with usage features
clusterset_usage <-
  dplyr::select(
    clusterset_log,
    key,
...
# Make clusterset only with network features
...
kmean_fun <- function(dataset, max_cluster) {
  ...
  require(foreach)
  require(doParallel)
  no_cores <- detectCores()-4 # go with 4 cores
  ...
  result.kmean <- foreach(i=2:max_cluster) %dopar% {
    km <- kmeans(dataset, centers = i, iter.max = 30, nstart = 15);
    return(km)
  }
  # stop parallel cores
  stopImplicitCluster()
...
# Trials
# kmean_ranged <- kmean_fun(clusterset_ranged[,-1], 10)
# kmean_ranged_nocor <- kmean_fun(clusterset_log_nocor[,-1], 10)
# kmean_log <- kmean_fun(clusterset_log[,-1], 10)
# kmean_log_nocor <- kmean_fun(clusterset_log[,-1], 10)

# Network and usage split. Run different trials and compare results
# check if results are constistent
kmean_network <- kmean_fun(clusterset_network[,-1], 20)
gc()
kmean_usage <- kmean_fun(clusterset_usage[,-1], 20)
gc()

# kmean_network2 <- kmean_fun(clusterset_network[,-1], 10)
# kmean_usage2 <- kmean_fun(clusterset_usage[,-1], 10)
# 
# kmean_network3 <- kmean_fun(clusterset_network[,-1], 10)
# kmean_usage3 <- kmean_fun(clusterset_usage[,-1], 10)
# 
# kmean_network4 <- kmean_fun(clusterset_network[,-1], 10)
# kmean_usage4 <- kmean_fun(clusterset_usage[,-1], 10)
# 
# kmean_network5 <- kmean_fun(clusterset_network[,-1], 10)
# kmean_usage5 <- kmean_fun(clusterset_usage[,-1], 10)
...
elbow_plot <- function(dataset, plotname) {
  plot(2:20, sapply(dataset, function(x) x$tot.withinss), type="b", xlab="Number of Clusters", ylab="Within groups sum of squares", main = plotname)
}

# show plot
elbow_plot(kmean_network, "Clusterset network")
elbow_plot(kmean_usage, "Clusterset usage")

# safe plot
png("elbow_kmean_usage.png", width = 500,  height = 400, res = 100)
par(mfrow=c(1,1))
elbow_plot(kmean_usage, "Clusterset usage")
dev.off()
# safe plot
png("elbow_kmean_network.png", width = 500,  height = 400, res = 100)
par(mfrow=c(1,1))
elbow_plot(kmean_network, "Clusterset network")
dev.off()
...
pamk_ranged <- pamk(clusterset_log[,-1], krange = 1:10, criterion = "ch", usepam = FALSE)
# # nr of cluster
# pamk_ranged$nc
...
clara_fun <- function(dataset, max_cluster) {
  ...
  result.clara <- foreach(i=2:max_cluster,.verbose = TRUE,
                          .packages = 'cluster') %dopar% {
                            pam_set <- clara(dataset, k = i, metric = "euclidean", stand = FALSE)
                            return(pam_set)
                          }
...
# run clara function
# clara_log <- clara_fun(clusterset_log[,-1], 10)
# clara_log_nocor <- clara_fun(clusterset_log_nocor[,-1], 10)

clara_usage <- clara_fun(clusterset_usage[,-1], 20)
clara_network <- clara_fun(clusterset_network[,-1], 20)

clare_clusterset <- clara_fun(clusterset_log[,-1], 15)

```


### Elbow technique for CLARA (PAM)
* Plot the objective
```{r}
elbow_objective <- function(dataset, plotname) {
  plot(2:15, sapply(dataset, function(x) x$objective), type="b", xlab="Number of Clusters", ylab="objective", main = plotname)
}

# show plot
elbow_objective(clare_clusterset, "Clusterset CLARA (all features)")
elbow_objective(clara_network, "Clusterset network")
elbow_objective(clara_usage, "Clusterset usage")

# safe plot
png("elbow_clara_objective_network.png", width = 500,  height = 400, res = 100)
par(mfrow=c(1,1))
elbow_objective(clara_network, "Clusterset network")
dev.off()

png("elbow_clara_objective_usage.png", width = 500,  height = 400, res = 100)
par(mfrow=c(1,1))
elbow_objective(clara_usage, "Clusterset usage")
dev.off()

png("elbow_clara_objective_ALL.png", width = 500,  height = 400, res = 100)
par(mfrow=c(1,1))
elbow_objective(clare_clusterset, "Clusterset ALL")
dev.off()
...
# }
# 
# skmeans_ranged <- skmeans_fun(sample_frac(clusterset_usage[,-1], size = 0.1), 10)

```

## DBSCAN
* dbscan fails after 2 days running (bad allocation of memory)
** meaning, its still short on memory ...
```{r, eval=FALSE, include=FALSE}
# library(dbscan) # used to get KNN distance plot
# library(fpc)    # used dbscan of the fpc package
# 
# kNNdistplot(clusterset_network[,-1], k =  5)
# kNNdistplot(clusterset_usage[,-1], k =  5)
# 
# 
# abline(h = 0.21, lty = 2)
# 
# dbscan_clusterset_log <- dbscan::dbscan(x = clusterset_log[,-1], eps = 0.4, borderPoints = TRUE)
...
## Evaluate clusters
K-means is all about centroids and variance.  therefore criterions such as Calinski-Harabasz are more focused here.
...
```{r}
cluster_metrics <- function(dataset, datacluster, method) {
  require(clusterSim)
  ...
  x_values = 2:15
  temp <- data.frame(x_values, ch_values, db_values)
  return(temp)
}

  
ch_plot <- function(dataset, plotname){
  png(paste0(deparse(substitute(dataset)),"_log_count_final_bis.png"), width = 800,  height = 400, res = 100)
  par(mfrow=c(1,2))
  plot(dataset$x_values, dataset$ch_values, type="b", xlab="Number of Clusters", ylab="Calinski-Harabasz", main = plotname)
  plot(dataset$x_values, dataset$db_values, type="b", xlab="Number of Clusters", ylab="Davies-Bouldin", main = plotname)
  dev.off()
}
...
#   CH <- index.G1(dataset[,-1], datacluster)
#   DB <- index.DB(dataset[,-1], datacluster) 
...
# Kmeans
# Stats
kmean_network_stats <- cluster_metrics(clusterset_network, kmean_network, "kmean")
kmean_usage_stats <- cluster_metrics(clusterset_usage, kmean_usage, "kmean")

# plot function
ch_plot(kmean_network_stats, "Kmean - Network")
ch_plot(kmean_usage_stats, "Kmean - usage")
# ch_plot(clusterset_network, kmean_network2, "Kmean - network", method = "kmean")
# ch_plot(clusterset_usage, kmean_usage2, "Kmean - usage", method = "kmean")

# CLARA
# Stats
clara_cluster_stats <- cluster_metrics(clusterset_log, clare_clusterset[c(1:14)], "clara")
clara_network_stats <- cluster_metrics(clusterset_network,clara_network[c(1:10)], "clara")
clara_usage_stats <- cluster_metrics(clusterset_usage, clara_usage[c(1:10)], "clara")

# plot function
ch_plot(clara_cluster_stats, "Clusterset  Clara")

ch_plot(clara_network_stats, "Clusterset network Clara")
ch_plot(clara_usage_stats, "Clusterset usage - Clara")
# ch_plot(clusterset_network, clara_network2, "Clusterset network Clara", method = "clara")
# ch_plot(clusterset_usage, clara_usage2, "Clusterset usage - Clara", method = "clara")


# Get stats for a specific cluster
# sink("db_ch_statistics_log_set.txt")
# measure_cluster(clusterset_log, kmean_ranged[[1]]$cluster)
...
# measure_cluster(clusterset_log, clara_ranged[[1]]$cluster)
...
# 
# # Get stats for a specific cluster
# sink("db_ch_statistics_clara_log_manual_set.txt")
# measure_cluster(clusterset_log_manual, clara_log_manual[[1]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[2]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[3]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[4]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[5]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[6]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[7]]$cluster)
# measure_cluster(clusterset_log_manual, clara_log_manual[[8]]$cluster)
# sink()
# 
# # Get stats for a specific cluster
# sink("db_ch_statistics_clara_network_set.txt")
# measure_cluster(clusterset_network, clara_network[[1]]$cluster)
# measure_cluster(clusterset_network, clara_network[[2]]$cluster)
# measure_cluster(clusterset_network, clara_network[[3]]$cluster)
# measure_cluster(clusterset_network, clara_network[[4]]$cluster)
# measure_cluster(clusterset_network, clara_network[[5]]$cluster)
# measure_cluster(clusterset_network, clara_network[[6]]$cluster)
# measure_cluster(clusterset_network, clara_network[[7]]$cluster)
# measure_cluster(clusterset_network, clara_network[[8]]$cluster)
# sink()
# 
# # Get stats for a specific cluster
# sink("db_ch_statistics_clara_usage_set.txt")
# measure_cluster(clusterset_usage, clara_usage[[1]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[2]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[3]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[4]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[5]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[6]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[7]]$cluster)
# measure_cluster(clusterset_usage, clara_usage[[8]]$cluster)
# sink()
# 
# # Get stats for a specific cluster
# sink("db_ch_statistics_kmean_usage_set.txt")
# measure_cluster(clusterset_usage, kmean_usage[[1]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[2]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[3]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[4]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[5]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[6]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[7]]$cluster)
# measure_cluster(clusterset_usage, kmean_usage[[8]]$cluster)
# sink()
# 
# # Get stats for a specific cluster
# sink("db_ch_statistics_kmean_network_set.txt")
# measure_cluster(clusterset_network, kmean_network[[1]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[2]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[3]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[4]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[5]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[6]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[7]]$cluster)
# measure_cluster(clusterset_network, kmean_network[[8]]$cluster)
# sink()
...
```{r}
# Make a dataset of the clusterset (ranges values) plus the clusternumber from the cluster results
clusterset_all_final <-
  cbind(clusterset_log, clara = clare_clusterset[[4]]$clustering)
clusterset_usage_final <-
  cbind(clusterset_usage, clara = clara_usage[[3]]$clustering, kmean = kmean_usage[[3]]$cluster)
clusterset_network_final <-
    cbind(clusterset_network, clara = clara_network[[2]]$clustering, kmean = kmean_network[[2]]$cluster)


# Make a dataset of the clusterset (ORIGINAL values) plus the clusternumber from the cluster results
clusterset_usage_orig_final <-
  cbind(dplyr::select_(clusterset, .dots = colnames(clusterset_usage)), clara = clara_usage[[3]]$clustering, kmean = kmean_usage[[3]]$cluster)

clusterset_network_orig_final <-
  cbind(dplyr::select_(clusterset, .dots = colnames(clusterset_network)),clara = clara_network[[2]]$clustering, kmean = kmean_network[[2]]$cluster)

clusterset_all_orig_final <-
    cbind(clusterset, clara = clare_clusterset[[4]]$clustering)


```

## Cluster interpretation: Make visual plot of cluster characteristics
* Look at cluster characteristics 
* to plot, used the scaled set
* (mean, var)

### Cluster interpretation on ranged clusterset
* maybe hard for interpretation, but easy to plot when everything is ranged 0-1
```{r}
clusterset_usage_mean <-
  clusterset_usage_final %>% dplyr::select(-key, -kmean) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
            mean_cluster = mean(value),
            var_cluster = var(value),
            nr_cluster = n())

clusterset_network_mean <-
  clusterset_network_final %>% dplyr::select(-key, -kmean) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
...
clusterset_all_mean <-
  clusterset_all_final %>% dplyr::select(-key) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
...
# plots
plot_cluster_analyse <- function(plotset, cluster_var, fill_var, title, name) {
      plotset %>%
      ggplot(., aes(x = as.factor(get(cluster_var)), y = variable, label = round(get(fill_var), 2))) + 
      geom_tile(aes(fill = get(fill_var)), color = "white") + geom_text(color = "gray50")+
      theme_minimal() + 
      scale_fill_gradient(low = "white", high = "#4AC29A", guide = guide_colorbar(title ="value")) + 
      theme_bw() + 
      theme(axis.ticks = element_blank(), panel.border = element_blank()) +
      labs(title = title) +
      scale_x_discrete(expand = c(0, 0)) +
      xlab("cluster") + 
      ggsave(filename = name, dpi = 150)
}

# PLOT MEAN of cluster CLARA
# usage
plot_cluster_analyse(clusterset_usage_mean, "clara", "mean_cluster", "Mean of usage clusters", "mean_usage_cluster.png"  )
# network
plot_cluster_analyse(clusterset_network_mean, "clara", "mean_cluster", "Mean of network clusters", "mean_network_cluster.png"  )
# ALL
plot_cluster_analyse(clusterset_all_mean, "clara", "mean_cluster", "Mean of clusters (all clusters)", "mean_all_cluster.png"  )

# PLOT VAR of cluster
# usage
plot_cluster_analyse(clusterset_usage_mean, "clara", "var_cluster", "Variance of usage clusters", "var_usage_cluster.png"  )
# network
plot_cluster_analyse(clusterset_network_mean, "clara", "var_cluster", "Variance of network clusters", "var_network_cluster.png"  )


## SAME FOR ORIGINAL SET
clusterset_usage_org_stat <-
  clusterset_usage_orig_final %>% dplyr::select(-key, -kmean) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
...
clusterset_network_org_stat <-
  clusterset_network_orig_final %>% dplyr::select(-key, -kmean) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
...
clusterset_network_org_stat <-
  clusterset_all_orig_final %>% dplyr::select(-key) %>%
  group_by(clara) %>%
  gather(variable, value, -clara) %>%
  group_by(clara, variable) %>%
  summarise(median_cluster = median(value),
...
# Or just make summary statistics
require(stargazer)
stargazer(filter(clusterset_network_orig_final, clara == 1), type = "text", title="Descriptive statistics", digits=1, out ="stats_network_clust1.txt")

clusterset_usage_orig_final 
clusterset_network_orig_final 

stargazer(filter(clusterset_all_orig_final, clara == 1), type = "text", title="Descriptive statistics", digits=1, out ="stats_clust1.txt")

# Plot overlap phenomemon
sample_frac(clusterset_usage_final, size = 0.008) %>% ggplot(., aes(x = total_call_time, y = nr_call_WEEK_perc, colour = clara)) + geom_point(alpha = 0.4) + theme_bw() + theme_minimal()
```
...
```{r}
clusterset_usage_mean[,c(1,6)] %>% unique() %>% transform(total = sum(nr_cluster)) %>% mutate(perc = round(nr_cluster / total * 100, 1))

clusterset_usage_orig_final %>% 
    separate(key, into = c("mygroup", "A_NUMBER")) %>%
  group_by(clara) %>%
  summarise(n_customers = n_distinct(A_NUMBER))


clusterset_network_mean[,c(1,6)] %>% unique() %>% transform(total = sum(nr_cluster)) %>% mutate(perc = round(nr_cluster / total * 100, 1))
...
* set minimal supportlevel on th itemset for the sequence mining

```{r}
library(TraMineR)
# Build set with proper key attributes
create_sequence_objects <- function(dataset) {
      # create dataset: mygroup, anumber , cluster 
      temp <- dplyr::select(dataset, key, clara) %>%
               separate(key, into = c("mygroup", "A_NUMBER"))
      
      # Build sequence set of clusters per mygroup
      seq_temp <-
        temp %>%
        spread(mygroup, clara, fill = NA) 

      # Make factors
      seq_temp = data.frame(lapply(seq_temp,as.factor))
      
      # Create state sequence object with TraMiner
      # handle missing value: or delete the begin/gap/end states when missing or replace with "missing" label
      seq_set <- seqdef(seq_temp[,-1], id = seq_temp[,1], left = "DEL", nr = "None")
      return(seq_set)
}

# Make sequence of usege / network 
usage_seq <- create_sequence_objects(clusterset_usage_final)
network_seq <- create_sequence_objects(clusterset_network_final)

# seq objects
usage_seqobj <- seqecreate(usage_seq)
network_seqobj <- seqecreate(network_seq)
# Search for frequent subsequences (based on prefix-tree-based search)
usage_subseq <- seqefsub(seqecreate(usage_seq), pMinSupport = 0.1)
network_subseq <- seqefsub(seqecreate(network_seq), pMinSupport = 0.1)
usage_subseq
network_subseq

# Plot frequent sub sequences
plot(usage_subseq[1:15], col = "#4AC29A", main = "Freq. sequences usage clusters" )
plot(network_subseq[1:15], col = "#4AC29A",  main = "Freq. sequences network clusters" )


# Plot state distributionsa
# USage
seqiplot(usage_seq, title = "Index plot (first 10 sequences)", withlegend = TRUE)
seqdplot(usage_seq, title = "State distribution plot", withlegend = TRUE)
seqfplot(usage_seq, title = "Sequence frequency plot", withlegend = FALSE, pbarw = TRUE)
seqlegend(dataset)
